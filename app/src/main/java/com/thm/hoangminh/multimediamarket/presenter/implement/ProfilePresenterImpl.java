...
public class ProfilePresenterImpl implements ProfilePresenter {
    ...
    private void loadCurrentUserMultimedia() {
        for (int i = 1; i < 5; i++) {
            final int finalI = i;
            purchasedProductRepository.findAllByUserIdAndCateId(userId, MainActivity.categories.get(i).getCateId(),
                    new ValueEventListener() {
                        @Override
                        public void onDataChange(DataSnapshot dataSnapshot) {
                            if (dataSnapshot.exists()) {
    ...
    public void redirectToProductActivity(int cateIndex) {
        ...
        bundle.putString(Constants.CateIdKey, MainActivity.categories.get(cateIndex).getCateId());
        bundle.putString(Constants.CateTitleKey, MainActivity.categories.get(cateIndex).getName());
